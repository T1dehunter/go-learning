...
func TestAll(test *testing.T) {
	...
	TestHashLiterals(test)
	TestHashIndexExpressions(test)
}
...
func TestArrayLiterals(testing *testing.T) {
	input := "[1, 2 * 2, 3 + 3]"

	evaluated := testEval(input)
	result, ok := evaluated.(*object.Array)
	if !ok {
		testing.Fatalf("object is not Array. got=%T (%+v)", evaluated, evaluated)
	}

	if len(result.Elements) != 3 {
		testing.Fatalf("array has wrong num of elements. got=%d", len(result.Elements))
	}

	testIntegerObject(testing, result.Elements[0], 1)
	testIntegerObject(testing, result.Elements[1], 4)
	testIntegerObject(testing, result.Elements[2], 6)
}
...
func TestHashIndexExpressions(testing *testing.T) {
	tests := []struct {
		input    string
		expected interface{}
	}{
